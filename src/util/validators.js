...
export const maxLengthWords = (message, maximumLength) => value => {
  if (!value) {
    return VALID;
  }
  const wordsCount = value.trim().split(/\s+/).length;
  const hasLength = value && typeof wordsCount === 'number';
  return hasLength && wordsCount <= maximumLength ? VALID : message;
};
...
export const autocompletePlaceOptionalSelected = message => value => {
  const selectedPlaceIsValid =
    !value ? true :
    value &&
    value.selectedPlace &&
