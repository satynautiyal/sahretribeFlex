...
import { DeclineReasonForm } from '../../forms';
...
export const TransactionPageComponent = props => {
  ...
  const userName = currentTransaction.customer?.attributes?.profile?.displayName;
  const userDisplayName = userName || '';

  const [isDeclineReasonFormOpen, setIsDeclineReasonFormOpen] = useState(false);

  const onSubmitEnquiry = values => {
    const { message } = values;
    onSendMessage(currentTransaction.id, message.trim())
      .then(() => {
        onDeclineSale(currentTransaction.id);
        setIsDeclineReasonFormOpen(false);
      })
      .catch(e => {
  ...
  const panel = isDataAvailable ? (
    <TransactionPanel
      ...
      onDeclineSale={() => {
        setIsDeclineReasonFormOpen(true);
      }}
      ...
      <LayoutSingleColumn>
        ...
        <LayoutWrapperMain>
          ...
          <Modal
            ...
            isOpen={isDeclineReasonFormOpen}
            onClose={() => setIsDeclineReasonFormOpen(false)}
            onManageDisableScrolling={onManageDisableScrolling}
          >
            <DeclineReasonForm
