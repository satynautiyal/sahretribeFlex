...
export const EmailVerificationPageComponent = props => {
  ...
  if (user && user.attributes.emailVerified) {
    setTimeout(() => <NamedRedirect name="LandingPage" />, 1000);
  }
  const { emailVerified, pendingEmail } = user.attributes;
  console.log('emailVerified', emailVerified, 'pendingEmail', pendingEmail);
  return (
      ...
      <LayoutSingleColumn>
        ...
        <LayoutWrapperMain className={css.layoutWrapperMain}>
          <div className={css.root}>
            ...
            {emailVerified && !pendingEmail && document.referrer.includes('android.gm') && (
              <Dialog />
            )}
          </div>
        </LayoutWrapperMain>
