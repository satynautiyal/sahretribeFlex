...
const tabLabel = (intl, tab) => {
  let key = null;
  if (tab === DESCRIPTION) {
    key = 'EditListingWizard.tabLabelPersonalDetails';
    // } else if (tab === FEATURES) {
    //   key = 'EditListingWizard.tabLabelFeatures';
  } else if (tab === BACKGROUND_AND_EXPERIENCE) {
    key = 'EditListingWizard.tabLabelBackground';
    // } else if (tab === TRAINING_PREFERENCES) {
    //   key = 'EditListingWizard.tabLabelTrainingPreferences';
    // } else if (tab === POLICY) {
    ...
    // } else if (tab === IDENTITY_VERIFICATION) {
    //   key = 'EditListingWizard.tabLabelIdentityVerification';
  } else if (tab === SUBMIT_APPLICATION) {
    key = 'EditListingWizard.tabLabelSubmitApplication';
  } else if (tab === REVIEW_AND_SUBMIT) {
    key = 'Review & Submit';
  }
...
const tabCompleted = (tab, listing) => {
  ...
  switch (tab) {
    case DESCRIPTION:
      return !!(
        (
          ...
          publicData &&
          publicData.gender &&
          publicData.usCitizen &&
          publicData.convicted &&
          privateData &&
          privateData.hearedAbout
        )
        // privateData.grantProleadDiscretion &&
        // privateData.grantProleadDiscretion.length
      );
    // case FEATURES:
    //   return !!(publicData && publicData.specializations);
    case BACKGROUND_AND_EXPERIENCE:
      return !!(
        publicData &&
        publicData.about &&
        // publicData.specializations &&
        publicData.category &&
        publicData.activity &&
        publicData.rules &&
        publicData.virtualTraining &&
        // publicData.convenientArea &&
        publicData.location &&
        // publicData.distance &&
        publicData.ageCategory
      );
    // case TRAINING_PREFERENCES:
    //   return !!(publicData && publicData.convenientArea && publicData.distance);
    // case POLICY:
    ...
    //     publicData.identityVerificationImagesUrls.length > 0
    //   );
    case SUBMIT_APPLICATION:
      return !!(
        privateData &&
        privateData.isInfoCorrect &&
        privateData.consentPayThroughProlead &&
        privateData.consentProleadsPolicy &&
        privateData.consentStripeAgreement &&
        privateData.consentGrantProleadLicense
      );
    default:
      return false;
  }
